...
import { DAY_TO_DAY_TITLE } from "./constants.js";
...
export default function App() {
  ...
  useEffect(() => {
    if (session) {
      Promise.all([fetchProjects(), fetchTasks(), fetchReminders()]).finally(() => setLoading(false));
    }
  }, [session]);
  ...
  async function fetchProjects() {
    ...
    const all = data || [];
    // Ensure Day-to-Day exists
    if (!all.find(p => p.title === DAY_TO_DAY_TITLE)) {
      const { data: dtd } = await supabase.from("projects").insert([{
        title: DAY_TO_DAY_TITLE, priority: "Medium", status: "In Progress",
        is_permanent: true,
      }]).select().single();
      if (dtd) setProjects([dtd, ...all]);
  ...
  // Sort projects: Day-to-Day pinned first
  const sortedProjects = [
    ...projects.filter(p => p.title === DAY_TO_DAY_TITLE),
    ...projects.filter(p => p.title !== DAY_TO_DAY_TITLE),
  ];

  const criticalCount = projects.filter(p => p.priority === "Critical" && p.status !== "Complete").length;
  const sharedProps   = { projects: sortedProjects, setProjects, tasks, setTasks, reminders, setReminders, fetchProjects, fetchTasks, fetchReminders, session };
    ...
    <div style={{ display:"flex",minHeight:"100vh",background:"#0C0C0E",fontFamily:"'DM Sans',sans-serif",color:"#E8E4DC" }}>
      ...
      <div style={{ width:collapsed?56:196,flexShrink:0,background:"#0A0A0D",borderRight:"1px solid #18181C",display:"flex",flexDirection:"column",transition:"width 0.2s ease",overflow:"hidden",position:"sticky",top:0,height:"100vh",zIndex:10 }}>
        <div style={{ padding:collapsed?"20px 0":"20px 18px",borderBottom:"1px solid #18181C",display:"flex",alignItems:"center",justifyContent:collapsed?"center":"space-between",minHeight:64 }}>
          {!collapsed&&<div><div style={{ fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:800,color:"#F0EBE0",lineHeight:1 }}>CFO</div><div style={{ fontFamily:"'DM Mono',monospace",fontSize:8,letterSpacing:3,color:"#3A3A3F",textTransform:"uppercase",marginTop:2 }}>Command</div></div>}
          <button onClick={()=>setCollapsed(v=>!v)} style={{ background:"none",border:"none",color:"#3A3A3F",cursor:"pointer",fontSize:14,padding:4,lineHeight:1,flexShrink:0 }}>{collapsed?"›":"‹"}</button>
        </div>
        <nav style={{ flex:1,paddingTop:10 }}>
          {NAV.map(item => {
            ...
            const badge  = item.id==="dashboard"?criticalCount:item.id==="reminders"?reminders.length:0;
            return (
              <button key={item.id} onClick={()=>setPage(item.id)} title={collapsed?item.label:""} style={{ display:"flex",alignItems:"center",gap:10,width:"100%",padding:collapsed?"12px 0":"10px 18px",justifyContent:collapsed?"center":"flex-start",background:active?"#161619":"transparent",border:"none",borderLeft:`2px solid ${active?"#E8E4DC":"transparent"}`,color:active?"#E8E4DC":"#444",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:active?600:400,transition:"all 0.15s",position:"relative" }}>
        ...
        <div style={{ padding:collapsed?"12px 0":"12px 18px",borderTop:"1px solid #18181C" }}>
          {!collapsed&&<div style={{ fontFamily:"'DM Mono',monospace",fontSize:9,color:"#2A2A2F",letterSpacing:0.5,marginBottom:8,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{session.user.email}</div>}
          <button onClick={handleSignOut} title="Sign out" style={{ background:"none",border:"none",color:"#2A2A2F",fontFamily:"'DM Mono',monospace",fontSize:9,letterSpacing:1,cursor:"pointer",textTransform:"uppercase",padding:0,display:"flex",alignItems:"center",gap:6,width:"100%",justifyContent:collapsed?"center":"flex-start" }}>
            <span style={{ fontSize:13 }}>⏻</span>{!collapsed&&"Sign Out"}
          </button>
        </div>
      </div>
      {/* Main */}
...
function Fonts() {
  return <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=DM+Mono:wght@400;500;600&family=Playfair+Display:wght@700;800&display=swap" rel="stylesheet" />;
}
