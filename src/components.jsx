...
export function DueBadge({ date }) {
  ...
  return (
    <span style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color, border: `1px solid ${color}`, borderRadius: 4, padding: "2px 7px", letterSpacing: 1, fontWeight: 600 }}>{label}</span>
  );
}
...
export function PriorityBadge({ priority }) {
  ...
  if (!pc) return null;
  return (
    <span style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: pc.color, background: pc.bg, border: `1px solid ${pc.color}33`, borderRadius: 4, padding: "2px 7px", letterSpacing: 1, textTransform: "uppercase" }}>{priority}</span>
  );
}
...
export function PrimaryBtn({ children, onClick, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{ background: disabled ? "#1A1A1E" : "#E8E4DC", color: disabled ? "#333" : "#0C0C0E", border: "none", borderRadius: 8, padding: "9px 18px", fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 12, cursor: disabled ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 6, transition: "all 0.15s" }}>
      {children}
    </button>
  );
}
...
export function GhostBtn({ children, onClick, color = "#888" }) {
  return (
    <button onClick={onClick} style={{ background: "transparent", color, border: `1px solid ${color}33`, borderRadius: 6, padding: "7px 13px", fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", transition: "all 0.15s" }}>
      {children}
    </button>
  );
}
...
export function SecondaryBtn({ children, onClick }) {
  return (
    <button onClick={onClick} style={{ background: "#17171A", color: "#888", border: "1px solid #2A2A2F", borderRadius: 8, padding: "9px 16px", fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
      {children}
    </button>
  );
}
...
export function CancelBtn({ onClick }) {
  return (
    <button onClick={onClick} style={{ flex: 1, padding: "11px 0", background: "#1A1A1E", color: "#555", border: "1px solid #2A2A2F", borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 13, cursor: "pointer" }}>Cancel</button>
  );
}

export function StatusToggle({ value, onChange }) {
  const { STATUS_OPTIONS } = require("./constants.js");
  return (
    <div style={{ display: "flex", gap: 8 }}>
      {["In Progress", "On Hold", "Complete"].map(s => (
        <button key={s} onClick={() => onChange(s)} style={{ flex: 1, padding: "8px 0", background: value === s ? "#E8E4DC" : "transparent", color: value === s ? "#0C0C0E" : "#555", border: `1px solid ${value === s ? "#E8E4DC" : "#2A2A2F"}`, borderRadius: 6, fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", transition: "all 0.15s" }}>{s}</button>
      ))}
    </div>
  );
}
